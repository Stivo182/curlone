...
Процедура ДобавитьЧтениеФайлов()
	...
	Код = УниверсальныеБлокиКода.ЧтениеТекстовыхФайлов(ОписаниеЗапроса.Файлы, ПрочитанныеФайлы);
...
Процедура ДобавитьДанныеЗапроса()
	...
	Если ВозможнаПередачаДанныхЧерезСтруктуру() Тогда
		
		РазделительКлючаИЗначения = "=";

		Конструктор
			.ДобавитьПустуюСтроку()
			.ДобавитьСтроку("%1 = Новый Структура();", ИмяПараметраДанныеЗапроса);
	...
	Иначе
		...
		ТелоЗапроса = УниверсальныеБлокиКода.ТелоЗапросаСтрокой(
				ОписаниеЗапроса.ОтправляемыеТекстовыеДанные, ПрочитанныеФайлы);
...
Функция ВозможнаПередачаДанныхЧерезСтруктуру()
