...
Перем ДанныеЗапросаСтрока; // Строка
...
Функция Получить(Описание, Ошибки = Неопределено) Экспорт
	...
	Для Каждого ОписаниеРесурса Из ОписаниеЗапроса.АдресаРесурсов Цикл
		...
		ДобавитьПоследовательнуюОтправкуДвоичныхДанных(ОписаниеРесурса);
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ТелоЗапроса) Тогда
			Если СтрЧислоСтрок(ТелоЗапроса) = 1 Тогда
				ДанныеЗапросаСтрока = ТелоЗапроса;
			Иначе
				Конструктор
					.ДобавитьПустуюСтроку()
...
Процедура ДобавитьПоследовательнуюОтправкуДвоичныхДанных(ОписаниеРесурса)
	...
	Для Каждого ПередаваемыйФайл Из ПередаваемыеФайлы Цикл
		...
		СборкаURL = УниверсальныеБлокиКода.СборкаURL(СтруктураURL);

		Конструктор.ДобавитьСтроку("%1 = Новый ДвоичныеДанные(%2);", 
			ИмяПараметраДанныеЗапроса, 
			Конструктор.ПараметрВСтроку(ПередаваемыйФайл.ИмяФайла));

		ДобавитьВызовМетода(ОписаниеРесурса, СборкаURL, ИмяПараметраДанныеЗапроса);
...
Процедура ДобавитьВызовМетода(ОписаниеРесурса, Знач URL = Неопределено, Знач ДанныеЗапроса = Неопределено)

	Если URL = Неопределено Тогда
		URL = ОписаниеРесурса.URL;
	КонецЕсли;

	Если ДанныеЗапроса = Неопределено И Состояние.ПереданоТелоЗапроса Тогда
		Если ЗначениеЗаполнено(ДанныеЗапросаСтрока) Тогда
			ДанныеЗапроса = ДанныеЗапросаСтрока;
		Иначе
			ДанныеЗапроса = ИмяПараметраДанныеЗапроса;
		КонецЕсли;
	КонецЕсли;

	ДобавитьДополнительныеПараметры(ДанныеЗапроса);
	
	Если Состояние.ЕстьДополнительныеПараметры Тогда
		Конструктор.ДобавитьПустуюСтроку();
	КонецЕсли;

	ПараметрURL = ?(Лев(URL, 1) = """", URL, Конструктор.ПараметрВСтроку(URL));
	...
	ИначеЕсли ОписаниеРесурса.Метод = "POST" Или ОписаниеРесурса.Метод = "PUT" Тогда
		...
		МетодКоннектора = ?(ОписаниеРесурса.Метод = "POST", "Post", "Put");
...
Процедура ДобавитьДополнительныеПараметры(ДанныеЗапроса)
